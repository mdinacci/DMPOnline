...
DMPOnline is a Ruby on Rails application and you'll need to have Ruby 1.9.2 or greater installed on your server and a MySQL server v5.0 or greater.

Further details on how to install Ruby on Rails applications are available from the ((<Ruby on Rails site|URL:http://rubyonrails.org>)).  

While not a prerequisite, we are running this using ((<Phusion Passenger 3|URL:http://www.modrails.com/>)) with an Apache webserver on Red Hat Enterprise Linux v5.

For the PDF conversion we use the shell utility ((<wkhtmltopdf|URL:http://code.google.com/p/wkhtmltopdf/>)) which will need to be installed on your server.  
